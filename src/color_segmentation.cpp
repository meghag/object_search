...
{
public:
  ColorSegmenter(ros::NodeHandle & n, int i): n_(n)
  {
    ...
    if (i == 0) {
      ...
      if (pcl::io::loadPCDFile<pcl::PointXYZRGB> ("object_cloud.pcd", *object_cloud) == -1) {
	...
	return;
      ...
      seg_sub_ = n_.subscribe("/object_cloud", 1, &ColorSegmenter::callback, this);
    }
  };
...
private:
  ...
  void cluster()
  {
    ...
    *object_cloud = objectCloud_;
    ...
    color_clustering(*object_cloud, *indices, tree, 0.03, cluster_indices, 1000, 8000);
  ...
  void color_clustering (const PointCloud<PointT> &cloud, const vector<int> &indices,
			 ...
			 unsigned int min_pts_per_cluster, unsigned int max_pts_per_cluster)
  {
	...
	while (sq_idx < (int)seed_queue.size ()) {
		...
		// Perform a simple Euclidean clustering. This is where the change is made for color
...
int main (int argc, char** argv)
{
  ...
  ros::NodeHandle n;
  ColorSegmenter cs(n,atoi(argv[1]));
  ros::spin();
  return 0;
}
