...
Manipulator::Manipulator(ros::NodeHandle & n): n_(n)
{
	...
	bbx_pub_ = n_.advertise<visualization_msgs::Marker>("target_bbx",10);
	...
	LEFT_RESET.x = 0.35;	//0.68;	//0.3;
	LEFT_RESET.y = 0.3; //0.7
	LEFT_RESET.z = 0.9; //1.1
	RIGHT_RESET.x = 0.35; //0.68;	//0.3;
	RIGHT_RESET.y = -0.3;  //-0.7
	RIGHT_RESET.z = 0.9;   //1.1
		...
		tum_os::Execute_Plan::Response &res)
{
	...
	for (size_t i = 0; i < object_to_move_.size(); i++)
	{
		...
		fromROSMsg(target_cloud2_, *objectCloud);
		tf::Vector3 min, max;
		...
		ROS_INFO("bbx position: %f %f %f, dimensions: %f %f %f", bbx_.pose_stamped.pose.position.x, bbx_.pose_stamped.pose.position.y,
				bbx_.pose_stamped.pose.position.z,
				bbx_.dimensions.x, bbx_.dimensions.y, bbx_.dimensions.z);
		...
		res.result = true;
		if (push[i]) {
			ROS_ERROR("This object has to be pushed.");
			if (!push_object(i))
				res.result = false;
		} else {
			if (!pick_n_place(i))
		...
		//gripper_.open();
		ROS_INFO("Moving left arm to reset position");
		move_arm_pick(active_reset_, 1, 1, 1);
...
vector<geometry_msgs::Point> Manipulator::waypoints(size_t idx)
{
	...
	//temp.x = 0.4;
	temp.x -= 0.1;
	temp.z = center.z + bbx_.dimensions.z*0.5 - 0.02;
	wp.push_back(temp);				//In front of the object
	temp.x = center.x - bbx_.dimensions.x*0.5 + 0.01;	//+ 0.01;	// + bbx_.dimensions.z*0.5 - 0.01;
	...
	temp.y = temp2.y;
	temp.z += 0.01;
	wp.push_back(temp);				//In front of destination
	temp2.x -= 0.07;
	temp2.z = temp.z;
	wp.push_back(temp2);			//Near the destination pose with the object in hand
	//temp2.z -= 0.01;
	temp2.x = dest_pose_[idx].position.x - bbx_.dimensions.x*0.5 + 0.01;	// + bbx_.dimensions.z*0.5;
	//temp2.z += 0.03;
...
bool Manipulator::pick_n_place(size_t idx)
{
	...
	ROS_INFO("Published target object cloud and bbx");

	ROS_INFO("Finding waypoints");
	vector<geometry_msgs::Point> wp = waypoints(idx);		//Arm waypoints

	for (size_t w = 0; w < wp.size(); w++) {
		visualization_msgs::Marker wp_marker = set_marker("base_link", "wp", w, visualization_msgs::Marker::SPHERE,
					wp[w], 0.02, 0.0f, 1.0f, 0.0f, 1.0);
		bbx_pub_.publish(wp_marker);
	}
	breakpoint();
	//return false;
	...
	//{
	if (dest_pose_[idx].position.y > 0) {
		ROS_INFO("Pick up with left arm.");
		active_arm_ = "left_arm";
		active_arm_sym_ = 'l';
		active_reset_ = LEFT_RESET;
	} else {
		ROS_INFO("Pick up with right arm.");
		active_arm_ = "right_arm";
		active_arm_sym_ = 'r';
		active_reset_ = RIGHT_RESET;
	}
	...
	for (size_t i = 0; i < wp.size(); i++)
	{
		...
		//if (i == 0 && (move_arm_pick(wp[i], 1, 0, 1) == 0))
		if (i == 0 || i == 3 || i > 6) {
			move_arm_pick(wp[i], 1, 0, 1);
			breakpoint();
			continue;
		} else if (i == 1 && (move_arm_pick(wp[i], 1, 0, 1) == 0)) {
			breakpoint();
			gripper_.close(active_arm_sym_);
			//gripper_.close();
		} else if (i == 2 && (move_arm_pick(wp[i], 1, 0,1) == 0)) {
			breakpoint();
		}// else if (i == 3 && (move_arm_pick(wp[i], 1, 0, 1) == 0)) {
		//	breakpoint();
		//}
		else if (i == 4 && (move_arm_pick(wp[i], 1, 0, 1) == 0)) {
			breakpoint();
		} else if (i == 5 && (move_arm_pick(wp[i], 1, 0, 1) == 0)) {
			gripper_.open(active_arm_sym_);
			//gripper_.open();
			breakpoint();
		} else if (i == 6 && (move_arm_pick(wp[i], 1, 0, 1) == 0)) {
			breakpoint();
		} else {
			success = false;
			...
			if (i > 1 && i < 5) {
				...
				if (temp.x < 0.5)
					temp.x = 0.5;
				move_arm_pick(temp, 1, 0, 1);
				gripper_.open(active_arm_sym_);
				//gripper_.open();
				temp.x -= 0.2;
				move_arm_pick(temp, 1, 0, 1);
			}
			move_arm_pick(active_reset_, 1, 0, 1);
			break;
		}
	}

	if (move_arm_pick(active_reset_, 1, 1, 1) != 0) {
		geometry_msgs::Point backup;
		backup.x = 0.4;
		backup.y = 0.0;
		backup.z = 0.8;
		move_arm_pick(backup, 1, 1, 1);
		move_arm_pick(active_reset_, 1, 1, 1);
	}
...
vector<geometry_msgs::Point> Manipulator::push_waypoints(size_t idx)	//(pcl::PointCloud<PointT> pcd, int push_direction,
		...
		//geometry_msgs::Point& final_point, geometry_msgs::Quaternion& gripper_orientation)
{
	...
	if (dest_pose_[idx].position.y > source_pose_[idx].position.y) {
		ROS_INFO("Push left");
		temp.x = center.x - 0.2;
		temp.y = center.y - bbx_.dimensions.y*0.5 - 0.05;
		wp.push_back(temp);				//In front of the object
		temp.x = center.x - 0.04;
		temp.y = center.y - bbx_.dimensions.y*0.5 - 0.05;
		wp.push_back(temp);
		wp.push_back(center);
		temp.y = temp2.y - bbx_.dimensions.y*0.5 - 0.05;
		wp.push_back(temp);
	} else if (dest_pose_[idx].position.y < source_pose_[idx].position.y) {
		ROS_INFO("Push right");
		temp.x = center.x - 0.2;
		temp.y = center.y + bbx_.dimensions.y*0.5 + 0.05;
		wp.push_back(temp);				//In front of the object
...
bool Manipulator::push_object(size_t idx)
{
	ROS_INFO("Inside push");
	target_pub_.publish(target_cloud2_);
	visualization_msgs::Marker bbx_marker = set_marker("base_link", "bbx", 1, visualization_msgs::Marker::CUBE,
			bbx_.pose_stamped.pose, bbx_.dimensions, 1.0f, 1.0f, 0.0f, 1.0);
	//bbx_pub_.publish(bbx_marker);
	...
	ROS_INFO("Finding waypoints");
	vector<geometry_msgs::Point> wp = push_waypoints(idx);		//Arm waypoints
	...
	//{
	if (dest_pose_[idx].position.y > 0) {
		ROS_INFO("Push with left arm.");
		active_arm_ = "left_arm";
		active_arm_sym_ = 'l';
		active_reset_ = LEFT_RESET;
	} else {
		ROS_INFO("Push with right arm.");
		active_arm_ = "right_arm";
		active_arm_sym_ = 'r';
		active_reset_ = RIGHT_RESET;
	}
	...
	for (size_t i = 0; i < wp.size(); i++)
	{
		...
		if (i == 0 || i > 4) {
			move_arm_push(wp[i], 1, 0, 1);
			breakpoint();
			continue;
		} else if (i == 1 && (move_arm_push(wp[i], 1, 0, 1) == 0)) {
			breakpoint();
		} else if (i == 2 && (move_arm_push(wp[i], 1, 0,1) == 0)) {
			breakpoint();
		} else if (i == 3 && (move_arm_push(wp[i], 1, 0, 1) == 0)) {
			breakpoint();
		} else if (i == 4 && (move_arm_push(wp[i], 1, 0, 1) == 0)) {
			breakpoint();
		} else {
			success= false;
			ROS_ERROR("Move failed");
			...
			if (i > 0) {
				//geometry_msgs::Point temp = wp[i-1];
				gripper_.open(active_arm_sym_);
				move_arm_push(active_reset_, 1, 0, 1);
			}
			break;
		}
	}
	//move_arm_pick(active_reset_, 1, 1, 1);
	if (move_arm_push(active_reset_, 1, 0, 1) != 0) {
		geometry_msgs::Point backup;
		backup.x = 0.4;
		backup.y = 0.0;
		backup.z = 0.8;
		move_arm_push(backup, 1, 0, 1);
		move_arm_pick(active_reset_, 1, 1, 1);
	}
	return success;
}
...
void Manipulator::add_attached_object(arm_navigation_msgs::AttachedCollisionObject& acob, int operation)
{
	...
	add_collision_object(acob.object, 0, 0);
	//acob.link_name = "r_gripper_r_finger_tip_link";
	acob.link_name = "base_link";
	acob.touch_links.push_back("r_end_effector");
}

void Manipulator::add_collision_object(arm_navigation_msgs::CollisionObject& cob, int operation, int action)
{
	...
	object.dimensions[0] = bbx_.dimensions.x + 0.05;
	if (action == 0)
		//pick and place
		object.dimensions[1] = bbx_.dimensions.y + 0.05;
	else
		//push: need large padding in x direction if push is in y direction bcoz after push the hand will be in collision
		object.dimensions[0] = bbx_.dimensions.y + 0.25;
	object.dimensions[2] = bbx_.dimensions.z;	// + 0.05;
	cob.shapes.push_back(object);

	cob.poses.push_back(bbx_.pose_stamped.pose);
}

int Manipulator::move_arm_pick(geometry_msgs::Point go_to, int cluster_id,
		int collision_object_operation, int plan_id)
{
	...
	if (plan_id == 1) {
		...
		ROS_INFO("Allowing collisions between left end effector and collision object.");
		if (active_arm_sym_ == 'l')
			collision_op("l_end_effector", collision_name_, 0, 1.0, cop);
		else if (active_arm_sym_ == 'r')
					collision_op("r_end_effector", collision_name_, 0, 1.0, cop);
	} else if (plan_id == 2) {
		//cop.object1 = cop.COLLISION_SET_ALL;
		//cop.object2 = cop.COLLISION_SET_ALL;
		ROS_INFO("Allowing collisions between all.");
	...
	desired_pose.pose.position.y -= 0.005;
	if (plan_id == 1)
		//desired_pose.pose.position.z = go_to.z + 0.002;
		desired_pose.pose.position.x = go_to.x - 0.15;	// - 0.08;
	else if (plan_id == 2)
		desired_pose.pose.position.z = go_to.z + 0.19;


	ROS_INFO("link_name: %s, group name: %s", desired_pose.link_name.c_str(), goalA.motion_plan_request.group_name.c_str());
	...
	desired_pose.absolute_position_tolerance.x = 0.03; //0.01
	desired_pose.absolute_position_tolerance.y = 0.01;
	desired_pose.absolute_position_tolerance.z = 0.03;  //0.01
	...
	if (active_arm_sym_ == 'r')
	{
		...
		finished_within_time = move_right_arm.waitForResult(ros::Duration(150.0));
		...
		finished_within_time = move_left_arm.waitForResult(ros::Duration(150.0));
		if (!finished_within_time) {
...
int Manipulator::move_arm_push(geometry_msgs::Point go_to, int cluster_id, int collision_object_operation, int plan_id)
{
	ROS_INFO("Inside move arm");
	//if (active_arm_sym_ == 'r') {
		actionlib::SimpleActionClient<arm_navigation_msgs::MoveArmAction> move_right_arm("move_right_arm",true);
	...
	if (plan_id == 1) {
		...
		//cop.object1 = "r_end_effector";
		//cop.object2 = collision_name_;
		ROS_INFO("Allowing collisions between end effector and collision object.");
		if (active_arm_sym_ == 'l')
			collision_op("l_end_effector", collision_name_, 0, 1.0, cop);
		else if (active_arm_sym_ == 'r')
			collision_op("r_end_effector", collision_name_, 0, 1.0, cop);
	...
	desired_pose.pose.orientation.x = 0.654036;
	desired_pose.pose.orientation.y = 0.0186624;
	desired_pose.pose.orientation.z = 0.13105;
	desired_pose.pose.orientation.w = 0.744792;
		...
		int collision_object_operation, bool attach, int plan_id, int action)
{
	...
	{
		desired_pose.pose.position.x -= 0.08;
		desired_pose.pose.position.y -= 0.01;
		desired_pose.pose.position.z += 0.01; //0.18;
	}
	...
	if ((action == 1 && plan_id == 2) || (action == 2 && plan_id < 2) || (action == 0 && plan_id == 2)) {
	...
	{
		move_right_arm.sendGoal(goalA);
		finished_within_time = move_right_arm.waitForResult(ros::Duration(100.0));
		if (!finished_within_time) {
			move_right_arm.cancelGoal();
			ROS_INFO("Timed out achieving goal A");
			return -1;
		...
		move_left_arm.sendGoal(goalA);
		finished_within_time = move_left_arm.waitForResult(ros::Duration(100.0));
		if (!finished_within_time) {
			move_left_arm.cancelGoal();
			ROS_INFO("Timed out achieving goal A");
			return -1;
